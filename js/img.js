window.onload = function()
{
	...
	for(var i=0; i<aDiv.length;i++)
	{
		...
		var cliWidth = (oW-aDiv[0].offsetWidth*aDiv.length)/(aDiv.length+1);
		...
		aDiv[i].onclick = function()
		{
			...
			var cliWidth = (oW-aImg[0].offsetWidth*aImg.length)/(aImg.length+1);
			
			startMove(this,{left:0,bottom:0,width:oW});
			...
			for(var j=0;j<aImg.length;j++)
			{
				...
				startMove(aImg[j],{left:Math.ceil(cliWidth + j*(cliWidth + _width))},function(obj){
					...
					aImg[j].onclick = function()
					{
						...
						oImg.onload=function ()
						{
							...
							oView.style.top = oH*1.2 + 'px';
							startMove(oView,{top:Math.ceil(oH/2)});
				...
				oPrev.onclick = function()
				{
					...
					startMove(oView,{top:-Math.ceil(oH/2)},function(){
						oView.style.top = Math.ceil(oH*1.2) + 'px';
						startMove(oView,{top:Math.ceil(oH/2)});
						iNow--;
				...
				oNext.onclick = function()
				{
					...
					startMove(oView,{top:-Math.ceil(oH/2)},function(){
						oView.style.top = Math.ceil(oH*1.2) + 'px';
						startMove(oView,{top:Math.ceil(document.oH/2)});
						iNow++;
						if(iNow==aImg.length)
						{
							iNow=0;
		...
		oBlack.onclick = function()
		{
			...
			var cliWidth2 = (oW-aDiv[0].offsetWidth*aDiv.length)/(aDiv.length+1);
